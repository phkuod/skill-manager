...
- **Real-LLM lane (opt-in)**: `skills/tests/test_ai_review_e2e_real.py` hits the live OpenRouter endpoint. Self-skips unless BOTH `AI_REVIEW_E2E_REAL=1` and an API key (`LLM_API_KEY` or `OPENROUTER_API_KEY`) are in the env, so the default suite stays hermetic. Chains exhausted purely by free-tier 429s `pytest.skip` rather than fail; assertions target verdict structure and the M4b extraction-regex round-trip, never the model's opinion.
...
- **Install modal UI smoke test**: `skills/static/skills/dev/install-modal-ui-audit.js`. After ANY change to `skill_detail.html` modal markup or `skill.js` modal logic, open `/skills/<some-skill>/` in a browser, paste the file contents into DevTools console, and run. In a real browser all checks must pass. Under headless Playwright a small set fails for environmental reasons (document.cookie writes are silently dropped — inject the cookie via the Playwright context instead — and INSTALL_TARGET bases without a {user_name} placeholder); compare against the previous run's baseline rather than zero. Catches the JIT-purged Tailwind trap (utility class missing from `skills/static/skills/vendor/tailwind.min.css`), modal positioning regressions, theme-contrast failures, and click-flow regressions. Also embeds well in the test plan for any install-feature PR.
