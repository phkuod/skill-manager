...
(function () {
  ...
  var uninstallBtn = document.getElementById('uninstall-button'); // legacy; pills handle uninstall now
  var installedInfoEl = document.getElementById('detail-installed-info');
  var confirmSlot = document.getElementById('detail-uninstall-confirm');
    ...
    fetchInstallTargets().then(function (targets) {
      ...
      var checks = targets.map(function (t) {
        return fetch('/api/install/targets/' + encodeURIComponent(t.name) + '/skills', { credentials: 'include' })
          ...
          .then(function (d) {
            if (d && d.catalog) {
              if (d.catalog.some(function (item) { return item.name === skillName; })) {
                foundTargets.push(t.name);
              }
            }
          })
          .catch(function () {});
      ...
      Promise.all(checks).then(function () {
        ...
        if (foundTargets.length > 0) {
          var html = '';
          foundTargets.forEach(function (tgt) { html += targetPillHtml(skillName, tgt); });
          installedInfoEl.innerHTML = html;
        } else {
          installedInfoEl.innerHTML = '';
          if (confirmSlot) {
            confirmSlot.classList.add('hidden');
  ...
  if (installedInfoEl) {
    installedInfoEl.addEventListener('click', function (ev) {
      var pill = ev.target.closest('.target-pill');
      if (!pill) return;
      ev.preventDefault();
      ev.stopPropagation();
      var targetName = pill.dataset.target;
      if (!targetName || !confirmSlot) return;
      openInlineUninstallConfirm(confirmSlot, skillName, targetName, function () {
